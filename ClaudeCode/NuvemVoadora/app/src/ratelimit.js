...
// Protege endpoints expostos a terceiros: login do portal (forca bruta de
// token) e rastreio publico (enumeracao de codigos sequenciais).
import { MuitasRequisicoes } from './erros.js';
...
export function criarLimitador({ max, janelaMs, agora = () => Date.now() }) {
  ...
  function middleware(req, _res, next) {
    if (!tentar(req.ip ?? 'desconhecido')) {
      return next(new MuitasRequisicoes('Muitas tentativas. Aguarde um instante e tente de novo.'));
    }
    next();
  }
